...
export default function PrediccionesPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/knockout?type=predictions&all=true&t=' + Date.now(), { cache: 'no-store' }).then(r => r.json()),
      fetch('/api/knockout?type=results&t=' + Date.now(), { cache: 'no-store' }).then(r => r.json()),
    ]).then(([p, pr, rs, kp, kr]) => {
      setPlayers(p.data || [])
      setAllPreds(pr.data || [])
      setResults(rs.data || [])
      setKnockPreds(kp.data || [])
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 20px 60px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {[['grupos', '⚽ Grupos'], ['eliminatorias', '🏆 Eliminatorias']].map(([tab, label]) => (
      ...
      {/* Group tabs */}
      {activeTab === 'grupos' && <div style={{ display: 'flex', gap: 6, overflowX: 'auto', marginBottom: 24, paddingBottom: 4 }}>
          ...
          {ALL_KNOCKOUT_ROUNDS.map(round => (
            <div key={round.id} style={{ marginBottom: 28 }}>
              ...
              {round.matches.map((match: KnockoutMatch) => {
                ...
                const matchPreds = players.map(player => ({
                  player,
                  pred: knockPreds.find(p => p.player_id === player.id && p.match_id === match.id) || null,
                }))
                  ...
                  <div key={match.id} style={{ marginBottom: 10 }}>
                    <div onClick={() => setExpandedMatch(isExpanded ? null : match.id)}
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>{res?.home_team || match.homeDesc}</span>
                        {res && <span style={{ margin: '0 8px', color: 'var(--gold)', fontFamily: "'Bebas Neue', sans-serif" }}>{res.home_score ?? '?'}-{res.away_score ?? '?'}</span>}
                        {!res && <span style={{ margin: '0 8px', color: 'var(--muted)' }}>vs</span>}
                        <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>{res?.away_team || match.awayDesc}</span>
                      </div>
                    ...
                    {isExpanded && (
                      <div style={{ background: 'rgba(17,17,24,0.7)', border: '1px solid rgba(244,197,66,0.2)', borderTop: 'none', borderRadius: '0 0 14px 14px', padding: '16px 18px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {matchPreds.map(({ player, pred }, idx) => (
                            <div key={player.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '10px 14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                              <div style={{ width: 32, height: 32, borderRadius: '50%', background: COLORS[idx % COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue', sans-serif", fontSize: '0.9rem', color: 'var(--dark)', flexShrink: 0 }}>
                                {player.username.charAt(0).toUpperCase()}
                              </div>
                              <span style={{ fontWeight: 600, fontSize: '0.88rem', minWidth: 80 }}>{player.username}</span>
