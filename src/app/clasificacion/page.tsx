...
```
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
    <div>
      <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '2.5rem', color: 'var(--gold)', letterSpacing: '0.06em', textShadow: '0 0 30px rgba(244,197,66,0.4)' }}>
        Clasificacion
      </h1>
      {lastUpdated && <p style={{ color: 'var(--muted)', fontSize: '0.75rem', marginTop: 2 }}>Actualizado: {lastUpdated.toLocaleTimeString('es-VE')}</p>}
    </div>
    <button onClick={load} style={{ background: 'rgba(17,17,24,0.8)', border: '1px solid rgba(255,255,255,0.08)', color: 'var(--text)', borderRadius: 10, padding: '8px 18px', cursor: 'pointer', fontFamily: "'Outfit', sans-serif", fontSize: '0.85rem', backdropFilter: 'blur(10px)' }}>
      Actualizar
    </button>
  </div>
              ...
              <div key={team.name} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, background: 'rgba(255,255,255,0.06)', borderRadius: 100, height: 8, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    background: i === 0 ? 'linear-gradient(90deg, var(--gold), var(--nike-red))' : i === 1 ? 'linear-gradient(90deg,#C0C0C0,#E8E8E8)' : 'linear-gradient(90deg,#CD7F32,#E8A87C)',
                    width: `${team.pct}%`, transition: 'width 0.6s ease',
                  }} />
                </div>
                ...
                <span style={{ fontSize: '0.72rem', color: 'var(--muted)', minWidth: 50 }}>{team.count} {team.count === 1 ? 'voto' : 'votos'}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {podiumOrder.map((p, vi) => {
            ...
            return (
              <div key={p.id} style={{ textAlign: 'center', flex: 1, maxWidth: 180 }}>
                <div style={{ width: 58, height: 58, borderRadius: '50%', background: COLORS[ci], display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.5rem', color: 'var(--dark)', margin: '0 auto 8px', border: `3px solid ${vi === 1 ? 'var(--gold)' : vi === 0 ? '#C0C0C0' : '#CD7F32'}`, boxShadow: vi === 1 ? '0 0 20px rgba(244,197,66,0.4)' : 'none' }}>
                ...
                <div style={{ marginTop: 8, borderRadius: '8px 8px 0 0', background: 'rgba(17,17,24,0.8)', border: '1px solid rgba(255,255,255,0.07)', height: podiumH[vi], backdropFilter: 'blur(8px)' }} />
              </div>
            )
          })}
        </div>
      ...
      {scores.length === 0 ? (
        ...
        <div style={{ background: 'rgba(17,17,24,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(12px)' }}>
          ...
          {scores.map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '44px 1fr 56px 80px 80px 70px', padding: '12px 18px', borderTop: '1px solid rgba(255,255,255,0.05)', background: isMe ? 'rgba(244,197,66,0.04)' : 'transparent', transition: 'background 0.15s' }}>
